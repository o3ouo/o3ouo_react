...
// 매개변수로 배열과 객체를 받음 > queryKey: [이름] 객체를 받고 배열 안에 api 이름 설정, queryFn: () => { return axion.get("API URL")}
...
function RQeury() {
  ...
  //   refetchOnMount: false, // 컴포넌트가 마운트가 될 때, 데이터를 가져올지의 여부를 제어하는 설정 / false: 처음 한 번만 호출하고 다른 컴포넌트로 이동했다 돌아와도 다시 호츨하지 않고 캐시에 남은 데이터를 사용한다. / always: stale 타입 여부에 관계없이 데이터를 계속 새로 가져오고 싶을 때 사용 
  ...
  //   enabled: false, // 초기 호출 여부 / 조건부로 쿼리를 실행할 때 / 기본값: true / false: api를 자동 호출 하지 않는다 (컴포넌트에 들어왔을 때에도 data를 가지고 있지 않음)
  // });
  ...
  // 에러가 났을 때 api 요청을 네 번 정도 새롭게 요청을 한다. > 그래도 실패 한다면 error를 내보냄
  ...
  return (
    <div>
      ...
      {/* 버튼을 클릭할 때마다 데이터를 가지고 올 수 있음 */}
      <button type="button" onClick={refetch}> Item show</button>
    </div>
  );
}

export default RQeury;
